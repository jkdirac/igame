...
SBMLResults_t *SBMLResults_create(Model_t *m, int timepoints)
{
  ...
  for ( i=0; i<Model_getNumSpecies(m); i++) {
    ...
    if (spe_idx >= num_species)
    {
	    printf("in sbmlResults.c num_species override array size\n");
	    break;
    }

    tc = results->species->tc[spe_idx];
    spe_idx++;
  ...
  for ( i=0; i<Model_getNumCompartments(m); i++) {
    ...
    if ( ! Compartment_getConstant(c) ) {
	    ...
	    //modified by jkdirac
	    if (cop_idx >= num_compartments)
	    {
		    printf("in sbmlResults.c num_compartments override array size\n");
		    break;
	    }

      tc = results->compartments->tc[cop_idx];
      cop_idx++;
  ...
  for ( i=0; i<Model_getNumParameters(m); i++) {
    ...
    if ( ! Parameter_getConstant(p) ) {
	    ...
	    //modified by jkdirac
	    if (par_idx >= num_parameters)
	    {
		    printf("in sbmlResults.c num_parameter override array size\n");
		    break;
	    }

      tc = results->parameters->tc[par_idx];
      par_idx++;
  ...
  for ( i=0; i<Model_getNumReactions(m); i++ ) {
    ...
    if (reac_idx >= num_reactions)
    {
	    printf("in sbmlResults.c num_reactions override array size\n");
	    break;
    }

    tc = results->fluxes->tc[reac_idx];
    reac_idx++;
